...
void processCommand()
{
    switch (SerialProtocol.getCommand().commandID)
    {
        ...
        case ML_SN:
            ...
            // encoder.setCount(SerialProtocol.getCommand().parameter *
            // encoderMotorstepsRelation);
        ...
        case ML_PO:
            ...
            // TemperatureSensor.setCompensationValue(SerialProtocol.getCommand().parameter
            // / 2.0);
            break;
        default:
            break;
...
void loop()
{
 ...
 if (!Motor.isInMove())
 {
   ...
   if (Motor.isTemperatureCompensationEnabled() && ((millis() - timestamp) >
 30000))
   {
